...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'KINGARTHURISREAL'
    answer = big_letters([(6, 1, 9),  # not a string
                          'KillIng mockiNGbirds?',  # KING
                          (10, 'lol', 'wrecked', 10),  # not a string
                          [],  # not a string
                          ['oops'],  # not a string
                          'Are youR Teeth Here Unually large?',  # ARTHU
                          ['OOPS'],  # not a string
                          'R !',  # R
                          '!',  #
                          'ooIS $$&*#%&&',  # IS
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k]) == str:
            for j in range(len(sequence_of_sequences[k])):
                    for h in range(len(sequence_of_sequences[k][j])):
                        if sequence_of_sequences[k][j][h].isupper():
                            new_string = new_string + str(sequence_of_sequences[k][j][h])
    return new_string
